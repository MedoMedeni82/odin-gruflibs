...
sendmmsg :: proc(fd: os.Handle, vmessages :[]MMsgHdr, flags: int) -> (int, os.Errno) {
    ...
    return int(sent), _b.get_c_errno(sent != -1);
}
...
recvmmsg :: proc(fd: os.Handle, vmessages: []MMsgHdr, tmo: ^_t.TimeSpec) -> (int, os.Errno) {
    ...
    return int(recvd), _b.get_c_errno(recvd != -1);
}

getsockopt :: proc(fd: os.Handle, level, optname: int, optval: rawptr, optlen: ^int) -> os.Errno {
    return _b.get_c_errno(_unix_getsockopt(_c.int(fd), _c.int(level), _c.int(optname), optval, cast(^_t.socklen_t) optlen) != -1);
}

setsockopt :: proc(fd: os.Handle, level, optname: int, $optval: rawptr, optlen: int) -> os.Errno {
    return _b.get_c_errno(_unix_setsockopt(_c.int(fd), _c.int(level), _c.int(optname), optval, _t.socklen_t(optlen)) != -1);
}

listen :: proc(fd: os.Handle, n: int) -> os.Errno {
    return _b.get_c_errno(_unix_listen(_c.int(fd), _c.int(n)) != -1);
}
...
accept :: proc(fd: os.Handle, addr: ^SockAddr, addr_len: ^int) -> (os.Handle, os.Errno) {
    ...
    return os.Handle(new_fd), _b.get_c_errno(new_fd != -1);
}
...
shutdown :: proc(fd: os.Handle, how: int) -> os.Errno {
    return _b.get_c_errno(_unix_shutdown(_c.int(fd), _c.int(how)) != -1);
}

sockatmark :: proc(fd: os.Handle) -> os.Errno {
    return _b.get_c_errno(_unix_sockatmark(_c.int(fd)) != -1);
}

isfdtype :: proc(fd: os.Handle, fdtype: int) -> os.Errno {
    return _b.get_c_errno(_unix_isfdtype(_c.int(fd), _c.int(fdtype)) != -1);
}